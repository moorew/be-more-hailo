...
def gemini_chat(messages: list, temperature: float = 0.4, max_tokens: int = 200) -> str:
    """Fallback chat using the Gemini REST API when local LLM is unreachable.
    ...
    Returns:
        The assistant's reply text, or None if Gemini is unavailable.
    """
    ...
    if not GEMINI_API_KEY:
        _logger.warning("[GEMINI] No API key configured – cannot fallback")
        return None
    ...
    try:
        _logger.info(f"[GEMINI] Falling back to Gemini ({GEMINI_MODEL})...")
        ...
        if resp.status_code == 200:
            ...
            if candidates:
                ...
                if text:
                    _logger.info(f"[GEMINI] Got response ({len(text)} chars)")
                    return text
            _logger.warning(f"[GEMINI] Empty response from API")
            return None
        else:
            _logger.error(f"[GEMINI] API error {resp.status_code}: {resp.text[:200]}")
            return None
